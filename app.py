...
def haversine_array(lats, lons):
    """
    Vectorized detection of distances (m) between consecutive lat/lon points.
    """
    lat = np.radians(lats)
    lon = np.radians(lons)
    dlat = lat[1:] - lat[:-1]
    ...
    return np.concatenate(([0.0], c * 6371000))
...
if st.sidebar.button("🗑️ Clear Logs"): st.session_state['logs']=[]

# Inputs
uploaded = st.file_uploader("Upload CSV with TIME, LATITUDE, LONGITUDE, RSRP, BTS ID, OPERATOR", type='csv')
