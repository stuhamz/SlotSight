# ------------------------------------------------------------
#  SlotSight – Slot-based Signal Summary & Map
# ------------------------------------------------------------
...
# ------------------------------------------------------------
@st.cache_data
def haversine_array(lats, lons):
    """
    Vectorised distance (metres) between consecutive lat/lon points.
    """
    lat = np.radians(lats)
    lon = np.radians(lons)
    ...
    return np.concatenate(([0.0], c * 6_371_000))  # Earth radius in metres


def interpolate_path(lats, lons, dists, target):
    """
    Linear interpolation of lat/lon at a given cumulative distance.
    """
    for j in range(len(dists) - 1):
        if dists[j] <= target <= dists[j + 1]:
            frac = (target - dists[j]) / (dists[j + 1] - dists[j])
            lat = lats[j] + frac * (lats[j + 1] - lats[j])
            lon = lons[j] + frac * (lons[j + 1] - lons[j])
            return lat, lon
...
#  App UI
# ------------------------------------------------------------
st.set_page_config(page_title="Slot-based Signal Analysis", layout="wide")
st.title("📊 Slot-based Signal Summary & Map")
...
uploaded = st.file_uploader(
    "Upload CSV with TIME, LATITUDE, LONGITUDE, RSRP, BTS ID, OPERATOR", type="csv"
)
slot_len = st.number_input("Slot length (m)", 0.1, 1000.0, 5.0)
thresh = st.number_input("Signal threshold (dBm)", -200.0, 0.0, -124.0)
...
if uploaded:
    try:
        ...
        log(f"Loaded {len(df0)} rows", "INFO")
        ...
        if "TIME" in df0.columns:
            ...
            df0["time_sec"] = df0["TIME"].apply(parse_time)
            df0 = df0.sort_values("time_sec").reset_index(drop=True)
            log("Sorted by TIME", "INFO")
        else:
            log("No TIME column; using file order", "WARNING")

        # 2 · Operator filter + weak-BTS purge
        ...
        drop_ids = [
            b for b in df0["BTS ID"].unique() if (df0[df0["BTS ID"] == b]["RSRP"] < thresh).all()
        ]
        if drop_ids:
            before2 = len(df0)
        ...
        # 3 · Prepare samples
        df = df0.rename(
            columns={"LATITUDE": "Lat", "LONGITUDE": "Long", "RSRP": "Signal Strength", "BTS ID": "BTS"}
        )
        ...
        num_good = grp.apply(lambda x: (x >= thresh).sum()).reindex(
            range(1, n_slots + 1), fill_value=0
        ).to_numpy()
        num_bad = num_samples - num_good

        # 8 · Slot start/end coordinates
        ...
        summary = pd.DataFrame(
            {
                ...
                "start_lat": start_pts[:, 0],
                "start_lon": start_pts[:, 1],
                "end_lat": end_pts[:, 0],
                "end_lon": end_pts[:, 1],
        ...
        # enforce first/last slot exact coords
        ...
        # 10 · Collapse consecutive ZERO slots
        status = summary["Status"].to_numpy()
        i = 0
        while i < n_slots:
            if status[i] == "ZERO":
                j = i
                while j + 1 < n_slots and status[j + 1] == "ZERO":
                    j += 1
                if j == i:
                    status[i] = "SKIP"
                else:
        ...
        deck = pdk.Deck(
            ...
            initial_view_state=pdk.ViewState(
                latitude=summary["start_lat"].mean(),
                longitude=summary["start_lon"].mean(),
                zoom=14,
                pitch=0,
            ),
        ...
        st.download_button(
            "Download Slot Summary CSV", csv, "slot_summary.csv", "text/csv"
        )
